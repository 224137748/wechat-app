...
export default class Lyric {
  ...
  _initLines() {
	...
	const tlyLines = this.tlyric ? this.tlyric.split('\n') : null 
    const offset = parseInt(this.tags['offset']) || 0
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i]
	  // 翻译歌词
	  const tlyLine = this.tlyLines ? this.tlyLines[i] : ''
      let result = timeExp.exec(line)
      if (result) {
        const txt = line.replace(timeExp, '').trim()
		const tlyTxt = !!tlyLine ? tlyLine.replace(timeExp, '').trim() : ''
		const time = result[1] * 60 * 1000 + result[2] * 1000 + (result[3] || 0) * 10 + offset
		const duration = i === 0 ? time : (time - this.lines[this.lines.length -1].time)
        if (txt) {
          this.lines.push({
            time,
            txt,
  ...
  _callHandler(i) {
	...
	const {txt, duration, tlyTxt= ''} = this.lines[i]
    this.handler({
      txt,
	  duration,
	  tlyTxt,
